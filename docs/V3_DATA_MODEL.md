...
| `employees` | 员工/会员 | id, tenant_id, site_id, name, easybr_browser_id |
| `workstations` | 执行设备 | id, tenant_id, site_id, name, agent_token, status, last_heartbeat_at |
...
- 超过阈值（如 60 秒）未上报，云端标记 workstation 为 `offline`。
- `offline` 状态下的 workstation 不再被分配新任务。
